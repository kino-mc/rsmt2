...
Print traits are in the [`to_smt` module][to smt mod], while the parse traits
are in the [`parse` module][parse mod].
...
These types are defined in the [`simple_example` module][simple example mod],
and will be imported from there in the rest of the explanation. We then define
the expression type, and make it implement the [`Expr2Smt` trait][expr 2 smt]
that writes it as an SMT-LIB 2 expression in a writer.

## Print functions

Since the structure for S-expressions is provided by users, they also need to
provide functions to print it in SMT Lib 2.
...
can pass some information to, say, `assert` that can modify printing. This is
typically used when dealing with transition systems to perform "print-time
unrolling". See the [`example` module][example mod] if you're interested; the
...
For convenience, all the `...2Smt` traits are implemented for `& str`. This is
useful for testing and maybe *very* simple application. Here, we won't
implement `Sym2Smt` or `Sort2Smt` and rely on `& str` for symbols and sorts.
Using a solver then boils down to creating a solver `Kid` representing the
process itself, and then (here) a `PlainSolver` that's just a wrapper around
the `Kid` proving functions from the `Solver` trait.

```
extern crate rsmt2 ;

use rsmt2::solver ;
use rsmt2::{ Kid, Solver } ;
...

{
  ...
  let mut solver = solver(& mut kid, ()).expect(
...
Note the `unit` parameter that we passed to the `solver` function: `solver(&
mut kid, ())`. This is actually the parser the solver should use when it needs
...
would have complained by saying that our parser `()` does not implement the
right parsing traits.

## The parser

This example will only use `get_model`, which only requires `IdentParser` and
`ValueParser`. In most cases, an empty parser `struct` with the right
...
As a side note, it would have been simpler to implement `ValueParser` with a
[`& mut SmtParser`][smt parser], as it provides the parsers we needed.
...
use rsmt2::{ solver, SmtRes, Kid, Solver } ;
use rsmt2::conf::z3 ;
